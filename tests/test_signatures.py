...
def test_apksigner_uses_signer_certificate_and_excludes_source_stamp(monkeypatch) -> None:
    output = (
        "Verifies\n"
...
def test_apksigner_supports_rotated_signer_sdk_ranges(monkeypatch) -> None:
    output = (
        "Verifies\n"
        "Verified using v3 scheme (APK Signature Scheme v3): true\n"
        f"Signer (minSdkVersion=33, maxSdkVersion=2147483647) "
        f"certificate SHA-256 digest: {'a' * 64}\n"
        f"Signer (minSdkVersion=24, maxSdkVersion=32) "
        f"certificate SHA-256 digest: {'b' * 64}\n"
        f"Source Stamp Signer certificate SHA-256 digest: {'c' * 64}\n"
    )
    monkeypatch.setattr(
        scanner,
    ...
    assert result["verified"] is True
    assert result["certificateSha256"] == ["A" * 64, "B" * 64]
