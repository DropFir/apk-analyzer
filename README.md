...
- 大型游戏 APK 条目超过 20,000 个时会提示复核但继续解析；只有超过 250,000
  个的极端条目数量才单独阻断，展开体积、压缩比和路径等安全限制仍始终生效。
...
- 图标格式和文件有效性；像素宽高及是否为正方形只记录，不作为阻塞条件。

静态扫描不联网、不安装 APK、不执行 APK，也不修改原件。只有点击“连接手机取证”后，工具才会把扫描通过的安装包安装到下拉框中明确选择的手机；它不会自动截图或录屏。工具不是杀毒软件，不会给出“安全”或“官方来源”的承诺。

若旧 APK 未声明 `targetSdkVersion`，工具会保留该缺失事实，并按 Android
安装器的判定值 0 检查低目标 SDK 限制；只有编辑在风险提示中明确选择“兼容安装”
后才使用 ADB 的兼容参数，确认内容和原始 Manifest 值都会写入交接记录。
...
6. 程序自动读取前基线之后、结束边界之前的媒体，并打开本地审查窗口。逐张勾选本次截图，检查录屏代表帧或完整回放，并确认“可见”“黑屏/禁止录屏”或“部分受保护”。如果人工确认安装包为 MOD，勾选“这是 MOD”并按需填写英文 `modInfo`（例如 `MOD, Unlocked Content`）。
...
人工确认是 MOD 包时，命令行完成取证可增加 `--mod`，并按需填写英文说明：

```powershell
  --mod --mod-info "MOD, Unlocked Content"
```
